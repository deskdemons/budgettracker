...
public:
    ...
    Textbox(sf::Vector2f position, sf::Vector2f dimension, int radius, std::string hintText, sf::Font &hintFont,
            int hintFontSize, sf::Color hintFontColor, std::string dataType, sf::Font &font, int fontSize, int maxLen,
            sf::Color fontColor, sf::Color bgColor, int borderThickness, sf::Color borderColor, int paddingLeft,
            int paddingTop); //this is constructor //position, dimension, radius, font, size of txt, max length of txt, txtColor, bgColor, outLineThickness, outLineColor

    void setSelected(
            bool);    //sets true if given t_amount_text_box is selected by using mouse click over the t_amount_text_box
    bool isFocus();    // returns boolean variable "selected"
    void
    typedOn(sf::Event); // every event is send here and if event is keypress, then it does its internal work as defined
