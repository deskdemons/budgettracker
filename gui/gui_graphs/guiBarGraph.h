...
public:
    ...
    void setBarValues(
            std::vector<double> barValues);   //vector of values of bar. for ex: std::vector<double> barVal = ({2000,4000,1000,8000}); values are basically used for magnitude of the bar height. Length of values, colors, and eachbarbottomtitle vectors should be same
    void setBarColors(
            std::vector<sf::Color> colors);   //for ex: std::vector<sf::Color> col = ({sf::Color::Red,sf::Color::Blue,sf::Color::Green,sf::Color::Yellow});
    void setEachBarBottomTitle(
            std::vector<std::string> eachBarBottomTitle); //for ex: std::vector<std::string> bottomtitle = ({"Jan","Feb","Mar","Apr});
    void setLabelTexts(
    ...
    void setPosition(
            sf::Vector2f position);    // ex: sf::Vector2f(200, 100).  sets the position of the top left cornor cor bar graph, i.e, the top of vertical scale
    ...
    void setBarMaxHeight(
            int barMaxHeight); // set what should be height of tallest bar. other values like scale dimension depends upon max Height
    ...
    void setTitlePadding(
            sf::Vector2f titlePadding);    //ex: sf::Vector(10, 10)  . set padding to left and top for bottom title of bar.

    void setLabelPosition(sf::Vector2f labelPosition);  //absolute position of a bunch of label texts
    void
    setLabelBoxDimension(sf::Vector2f labelBoxDimension);  //ex:: sf::Vector2f(16,16) .label box dimension width, height
    ...
    void setBarTopTextFont(
            sf::Font barTopTextFont);    //text on top of each bar. its font. actual bar top text is just magnitude of the bar
    ...
    void
    drawTo(sf::RenderWindow &window);  //just draws bar graph in buffer. note that we need to display drawn artifacts into the screen too, from main.cpp file
};
