...
void RoundedRectangle::drawer() {
	...
	for (int i = 1; i < 90; i++) {
		convex.setPoint(i, sf::Vector2f(position.x + radius - (radius * sin(i * unitDegree)), position.y + radius - (radius * cos(i * unitDegree)) ));
	}
	...
	for (int j = 92; j < 181; j++) {
		convex.setPoint(j, sf::Vector2f(position.x + radius - (radius * sin((j - 1) * unitDegree)),position.y + dimension.y - radius - (radius * cos((j - 1) * unitDegree)) ));
	}
	...
	for (int k = 183; k < 272; k++) {
		...
		convex.setPoint(k, sf::Vector2f(position.x + dimension.x - radius - (radius * sin((k - 2) * unitDegree)), dimension.y + position.y - radius - (radius * cos((k - 2) * unitDegree)) ));
	}
	...
	for (int l = 274; l < 363; l++) {
		...
		convex.setPoint(l, sf::Vector2f(position.x + dimension.x - radius - (radius * sin((l - 3) * unitDegree)), position.y + radius - (radius * cos((l - 3) * unitDegree)) ));
	}
