...
GuiPieChart::GuiPieChart() {
    ...
    //for label
    std::string txtLab[] = {"LabelText 1","LabelText 2","LabelText 3","LabelText 4","LabelText 5","LabelText 6","LabelText 7","LabelText 8","LabelText 9","LabelText 10","LabelText 11","LabelText 12"};
...
GuiPieChart::GuiPieChart(double radius, sf::Vector2f centerPosition, std::vector<sf::Color> colors, std::vector<double> percentages, std::vector<std::string> labelTexts, sf::Vector2f labelPosition, sf::Vector2f labelBoxDimension, int labelVerticalSpacing, int labelBoxRadius, sf::Font& labelFont, int labelFontSize, sf::Color labelFontColor, int labelBoxOutLineThickness, sf::Color labelBoxOutLineColor) {
    this->radius = radius;
...
void GuiPieChart::drawer() {
    ...
    for (int i = 0; i < length; i++) {
        ...
        for (int j = m; j < round(c); j++) {
            fan[i][k + 1].position = sf::Vector2f(position.x + (radius * sin(j * unitDegree)), position.y + (radius * cos(j * unitDegree)));
