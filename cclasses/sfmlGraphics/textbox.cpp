...
Textbox::Textbox() {	//default constructor
	...
	hint.setPosition(sf::Vector2f(position.x + paddingL, paddingT + position.y + ((dimension.y) / 2) - (hintFontSize / 1.5f) ));
	hint.setFont(hintFont);
	hint.setCharacterSize(hintFontSize);
	hint.setFillColor(hintFontColor);

	textbox.setPosition(sf::Vector2f(position.x + paddingL, paddingT + position.y + ((dimension.y) / 2) - (fontSize / 1.5f) ));
