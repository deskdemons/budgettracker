...
void Money::deserialize(const std::string &moneyStr) {
    ...
    Currency c(
            curTyp); //if curType passed in deserializer is invalid, then the throwing mechanism inside Currency throw error here and again it automatically throws error in deserliazser call point. just like multiple levels of throwing
    value = monVal;
    type = typ;
    currency = c;
